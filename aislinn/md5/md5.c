 ...
 *
 * 2014 - Stanislav Bohm: Migrated to Valgrind libc
 *
 ...
 */
static const void *body(AN_(MD5_CTX) *ctx, const void *data, unsigned long size)
{
...
void AN_(MD5_Init)(AN_(MD5_CTX) *ctx)
{
        ctx->a = 0x67452301;
        ctx->b = 0xefcdab89;
        ctx->c = 0x98badcfe;
...
void AN_(MD5_Update)(AN_(MD5_CTX) *ctx, const void *data, unsigned long size)
{
...
void AN_(MD5_Final)(MD5_Digest *result, AN_(MD5_CTX) *ctx)
{
        ...
        if (available < 8) {
                VG_(memset)(&ctx->buffer[used], 0, available);
                body(ctx, ctx->buffer, 64);
                used = 0;
                available = 64;
        }

        VG_(memset)(&ctx->buffer[used], 0, available - 8);
